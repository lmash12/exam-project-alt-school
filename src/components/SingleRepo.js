  ...
  function ShowSingleCard(){
    ...
    return (
      <div className="singleCard">
        ...
        {ShowSingleCard.description && <div className="showSingle">
            <label>Description:</label>
            <h3 style={{fontSize: "18px"}}>{ShowSingleCard.description}</h3>
            </div>}
            {ShowSingleCard.created_at && <div className="showSingle">
            <label>Created:</label>
            <h3 style={{fontSize: "18px"}}>{ShowSingleCard.created_at}</h3>
            </div>}
            {ShowSingleCard.language && <div className="showSingle">
            <label>Langauage:</label>
            <h3 style={{fontSize: "18px"}}>{ShowSingleCard.language}</h3>
            </div>}
        <div className="showSingle">
          <label>{`Open In GitHub:`}</label>
          <a target="_blank" href={ShowSingleCard.html_url}>{ShowSingleCard.name}</a>
        </div>
      </div>
     )
  }
